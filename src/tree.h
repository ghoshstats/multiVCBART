...
// ---- Backward-compatible aliases (older code expects these) ----
  bool is_bot() const { return (l == 0); }                 // leaf iff no children
  double get_c() const { return rule.c; }
  std::set<int> get_l_vals() const { return get_lvals(); } // alias
  std::set<int> get_r_vals() const { return get_rvals(); } // alias
  ...
  tree_cp get_ptr(int nid) const;   // const overload
